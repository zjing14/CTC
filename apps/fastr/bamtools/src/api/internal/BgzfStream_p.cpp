...
static void *Deflate_T(void *data);
static void *Inflate_T(void *data);
...
DataBlock::DataBlock(unsigned int uncompressed_block_size, unsigned int compressed_block_size, bool is_write_compressed)
    : UncompressedBlockSize(uncompressed_block_size)
    , CompressedBlockSize(compressed_block_size)
    , IsWriteCompressed(is_write_compressed)
    , BlockOffset(0)
{
    try {
        CompressedBlock   = new char[CompressedBlockSize];
        ...
        fprintf(stderr, "DataBlock ERROR: unable to allocate memory\n");
        exit(1);
    }
}
...
DataBlock::~DataBlock(void) {
    ...
    for(unsigned int i=0; i<WriteBuffers.size(); i++) {
        delete [] WriteBuffers[i];
    }
}

void DataBlock::CopyData(char* data, unsigned int size) {
    BlockOffset = size;
    memcpy(UncompressedBlock, data, size);
}

void DataBlock::Deflate() {
    while( BlockOffset > 0) {
        DeflateBlock();
    }
}

int DataBlock::DeflateBlock(void) {
    // initialize the gzip header
    char* buffer = CompressedBlock;
    memset(buffer, 0, 18);
    buffer[0]  = Constants::GZIP_ID1;
    ...
    BufferLengths.push_back(compressedLength);
    char* write_buffer = new char[compressedLength];
    memcpy(write_buffer, CompressedBlock, compressedLength);
    WriteBuffers.push_back(write_buffer);
...
int DataBlock::GetBlockLength() {
    int blockLength = 0;
    for(unsigned int i=0; i<BufferLengths.size(); i++) {
         blockLength += BufferLengths[i];
    }
    return blockLength;
}

BgzfStreamMT::BgzfStreamMT(int num_threads) 
    : BgzfStream()
    , NumThreads(num_threads)
{
    BlocksPerThread = BLOCKS_PER_THREAD;
    NumBufferedBlocks = 0;
    BufferedBlocks = vector <DataBlock*> (BlocksPerThread * NumThreads, NULL);
    
    BlockGrid = new DataBlock**[NumThreads];
    for(unsigned int i=0; i<NumThreads; i++) {
        BlockGrid[i] = new DataBlock*[BlocksPerThread];
    }

//	ReadBufferedBlocks = queue <ReadDataBlock*> ();
}

BgzfStreamMT::~BgzfStreamMT(void) {
    for(unsigned int i=0; i<NumThreads; i++) {
        delete [] BlockGrid[i];
    }
    delete [] BlockGrid;
...
// decompresses the current block
int ReadDataBlock::InflateBlock() {
	ReadDataBlock* rb = this;
    // inflate the data from compressed buffer into uncompressed buffer
    z_stream zs;
    zs.zalloc    = NULL;
    zs.zfree     = NULL;
    zs.next_in   = (Bytef*)rb->CompressedBlock + 18;
    zs.avail_in  = rb->InflateBlockLength - 16;
    zs.next_out  = (Bytef*)rb->UncompressedBlock;
    zs.avail_out = rb->UncompressedBlockSize;
...
bool BgzfStreamMT::PopulateReadBuffer() {
	ReadDataBlock** rbs = new ReadDataBlock*[NumThreads * BLOCKS_PER_THREAD];
	for(unsigned int i = 0; i < NumThreads * BLOCKS_PER_THREAD; i++)
		rbs[i] = NULL;
	...
	for(unsigned int i = 0; i < NumThreads * BLOCKS_PER_THREAD; i++)
	{
	    ...
	    // read block header from file
	    int count = fread(header, 1, sizeof(header), Stream);
		ReadDataBlock* rb = new ReadDataBlock(UncompressedBlockSize, CompressedBlockSize, false);
	    // if block header empty
	    if ( count == 0 ) {
	        BlockLength = 0;
			delete[] rb->UncompressedBlock;
			delete[] rb->CompressedBlock;
			delete rb;
	        break;//true;
	    }
	    ...
	    // copy header contents to compressed buffer
	    int blockLength = BamTools::UnpackUnsignedShort(&header[16]) + 1;
	    char* compressedBlock = rb->CompressedBlock;
	    memcpy(compressedBlock, header, Constants::BGZF_BLOCK_HEADER_LENGTH);
	    int remaining = blockLength - Constants::BGZF_BLOCK_HEADER_LENGTH;
		//rb->BlockLength = blockLength;	
	    // read remainder of block
	    count = fread(&compressedBlock[Constants::BGZF_BLOCK_HEADER_LENGTH], 1, remaining, Stream);
	    if ( count != remaining ) {
	        fprintf(stderr, "BgzfStream ERROR: read block failed - could not read data from block\n");
		...
		rb->BlockAddress = blockAddress;
		rb->InflateBlockLength = blockLength;
		rbs[i] = rb;
		readBlocks++;
	}
    ...
    for(unsigned i=0; i<NumThreads; i++){
        ret = pthread_create(&threads[i], NULL, Inflate_T, (void *)(&rbs[i * BLOCKS_PER_THREAD]));
        if (ret) {
...
bool BgzfStreamMT::ReadBlock() {
	//If the queue is empty, repopulate it.
	if(ReadBufferedBlocks.empty())
	{
		if(!PopulateReadBuffer()){
			return false;
		}
	}
	...
	else
	{
		delete [] UncompressedBlock;
		delete [] CompressedBlock;
		ReadDataBlock* rb = ReadBufferedBlocks.front();
		BlockOffset = 0;
		BlockAddress = rb->GetBlockAddress();
		BlockLength = rb->GetBlockLength();
		UncompressedBlock = rb->UncompressedBlock;
		CompressedBlock = rb->CompressedBlock;
		delete rb;
		ReadBufferedBlocks.pop();
...
unsigned int BgzfStreamMT::Write(const char* data, const unsigned int dataLen) {
    ...
    while ( numBytesWritten < dataLen ) {
        ...
        if ( BlockOffset == blockLength ) {
            // Add buffered blocks
            DataBlock* block = new DataBlock(UncompressedBlockSize, CompressedBlockSize, IsWriteCompressed);
            block->CopyData(UncompressedBlock, BlockOffset);
            BufferedBlocks[NumBufferedBlocks++] = block;

            if(NumBufferedBlocks == BlocksPerThread * NumThreads) {
                FlushBlock();
            }
            // Update block data
            BlockOffset = 0;
...
void BgzfStreamMT::FlushBlock(void) {
    for(unsigned int i=0; i<NumThreads; i++) {
        for(unsigned int j=0; j<BlocksPerThread; j++) {
            BlockGrid[i][j] = NULL;
        }
    }

    vector <unsigned int> indexes(NumThreads, 0);
    for(unsigned i=0; i<NumBufferedBlocks; i++){
        int thread_idx = i % NumThreads;
        BlockGrid[thread_idx][indexes[thread_idx]++] = BufferedBlocks[i];
    }
    ...
    int ret;
    for(unsigned i=0; i<NumThreads; i++){
        ret = pthread_create(&threads[i], NULL, Deflate_T, (void *)(BlockGrid[i]));
        if (ret) {
            perror("Thread creation :");
            exit(-1);
        }
    ...
    // Write blocks
    for(unsigned int i=0; i<NumBufferedBlocks; i++) {
        DataBlock* block = BufferedBlocks[i];
        for(unsigned int j=0; j<block->BufferLengths.size(); j++) {
            int numBytesWritten = fwrite(block->WriteBuffers[j], 1, block->BufferLengths[j], Stream);
            if ( numBytesWritten != block->BufferLengths[j] ) {
                fprintf(stderr, "BgzfStreamMT ERROR: expected to write %u bytes during flushing, but wrote %u bytes\n",
                        block->BufferLengths[j], numBytesWritten);
                exit(1);
            }
        }
    }
    ...
    // Update BlockAddress
    for(unsigned int i=0; i<NumBufferedBlocks; i++) {
        BlockAddress += BufferedBlocks[i]->GetBlockLength();
    }

    // Cleanup
    for(unsigned int i=0; i<NumBufferedBlocks; i++) {
        delete BufferedBlocks[i];
    }

    NumBufferedBlocks = 0;
}

void BgzfStreamMT::Close(void) {
...
void *Inflate_T(void *data)
{
	ReadDataBlock** rbs = (ReadDataBlock**)data;
	for(int i = 0; i < BLOCKS_PER_THREAD; i++)
	{
		// decompress block data
		ReadDataBlock* rb = rbs[i];
		if(rb == NULL)
			break;
	
		int count = rb->InflateBlock();
		if ( count < 0 ) {
		    fprintf(stderr, "BgzfStream ERROR: read block failed - could not decompress block data\n");
		    return false;
		}
		rb->BlockLength = count;	
	}
	return NULL;
}

void *Deflate_T(void *data)
{
    DataBlock** blocks = (DataBlock**)data;
...
//READ DATA BLOCK
ReadDataBlock::ReadDataBlock(unsigned int uncompressed_block_size, unsigned int compressed_block_size, bool is_write_compressed)
    : UncompressedBlockSize(uncompressed_block_size)
    , CompressedBlockSize(compressed_block_size)
{
    try {
...
ReadDataBlock::ReadDataBlock(const ReadDataBlock& rb)
{
	UncompressedBlockSize = rb.UncompressedBlockSize;
	CompressedBlockSize = rb.CompressedBlockSize;
	UncompressedBlock = rb.UncompressedBlock;
	CompressedBlock = rb.CompressedBlock;

	BlockAddress = rb.BlockAddress;
	BlockLength = rb.BlockLength;
}
...
uint64_t ReadDataBlock::GetBlockAddress()
{
	return BlockAddress;
}

unsigned int ReadDataBlock::GetBlockLength()
{
	return BlockLength;
}
